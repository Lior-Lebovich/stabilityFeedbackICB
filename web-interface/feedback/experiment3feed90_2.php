...
$continueExp='experiment3feed90_2.php';

// the following 6 rows were added to keep workerID and experimentType:
...
$sql4 = "SELECT DISTINCT manHor FROM biasform WHERE ID = '".substr($ses_id, 0, 26)."'";
$row4=mysqli_fetch_array(mysqli_query($conn, $sql4));
$manHor=$row4['manHor'];
$sql5 = "SELECT DISTINCT manVer FROM biasform WHERE ID = '".substr($ses_id, 0, 26)."'";
$row5=mysqli_fetch_array(mysqli_query($conn, $sql5));
$manVer=$row5['manVer'];

$sql6 = "SELECT DISTINCT feedbackType FROM biasform WHERE ID = '".substr($ses_id, 0, 26)."'";
$row6=mysqli_fetch_array(mysqli_query($conn, $sql6));
$numberPartOfExp=$row6['feedbackType'];

// the vectors below define the "correct" response in impossible trials and manip=1. It's the opposite if manip=-1.
$arrayTrue1ImpHorRight = array(7,13,25,37,49,61,85,91,115,127,133,145,157,175,193,199,205,217,223,247,253,265,277,289,301,325,331,355,367,373,385,397,415,433,439,445,457,463);
$arrayTrue1ImpHorLeft = array(67,307);
$arrayTrue1ImpVerTop = array(16,28,40,46,58,64,88,100,118,124,136,142,154,172,190,208,214,226,232,256,268,280,286,298,304,328,340,358,364,376,382,394,412,430,448,454,466,472);
$arrayTrue1ImpVerBottom = array(76,316);

$arrayTrue1ImpHorRightString = serialize($arrayTrue1ImpHorRight);
$arrayTrue1ImpHorLeftString = serialize($arrayTrue1ImpHorLeft);
$arrayTrue1ImpVerTopString = serialize($arrayTrue1ImpVerTop);
$arrayTrue1ImpVerBottomString = serialize($arrayTrue1ImpVerBottom);


//determine horizontal manipulation:
if($manHor==-1){
	$impManipHor=-1; 
}
elseif($manHor==1){
...
//determine vertical manipulation:
if($manVer==-1){
	$impManipVert=-1;
}
elseif($manVer==1){
...
$arrayDevHorizontal = array(-5,3,5,0,2,1,0,1,-5,-2,4,-2,0,-2,2,4,4,-3,0,2,3,-2,-3,-5,0,-1,1,1,4,-1,0,-3,-4,0,5,-5,-4,4,1,3,-1,-3,0,-4,2,0,3,-4,2,5,-1,-2,3,5,-1,-4,5,0,-5,-3,-3,1,-2,0,-3,-3,0,1,2,3,-4,-2,0,-1,-1,2,5,-1,0,-2,1,-1,-4,4,-4,-4,4,0,4,2,-5,3,-2,-4,3,3,0,-3,-1,0,5,4,0,-5,1,-3,4,5,0,2,-5,0,-5,5,-5,5,-2,1,2,3,-5,3,5,0,2,1,0,1,-5,-2,4,-2,0,-2,2,4,4,-3,0,2,3,-2,-3,-5,0,-1,1,1,4,-1,0,-3,-4,0,5,-5,-4,4,1,3,-1,-3,0,-4,2,0,3,-4,2,5,-1,-2,3,5,-1,-4,5,0,-5,-3,-3,1,-2,0,-3,-3,0,1,2,3,-4,-2,0,-1,-1,2,5,-1,0,-2,1,-1,-4,4,-4,-4,4,0,4,2,-5,3,-2,-4,3,3,0,-3,-1,0,5,4,0,-5,1,-3,4,5,0,2,-5,0,-5,5,-5,5,-2,1,2,3);
...
<script>
var realTrialJS='<?php print $realTrial?>';	
var realTrialJSnum=<?php print $realTrial?>;
var numberPartOfExpJS='<?php print $numberPartOfExp?>';	
</script>
...
if($trial == 496){ 
	$signal=999;
	$continueExp='endexperiment3feed90_2.php';
}
elseif($trial%31 == 0){
	$signal=999;
	$checkIfSignal999=1;
...
<script type="text/javascript">		
	...
	function hideimageFixation() {
		...
		function checkKey(e) {
			...
			document.getElementById('buttonIDtop')!=null && 
			document.getElementById('buttonIDtop').disabled==false)
			{
			...
			if(document.getElementById('buttonID')!=null && 
			document.getElementById('buttonIDright')!=null && 
			document.getElementById('buttonIDright').disabled==false)
			{
				if (e.keyCode == '32' && document.getElementById('buttonID').disabled==true) {//'spacebar'
					document.getElementById('buttonID').style.display = 'block'; // the submit button appears, but is hidden. If we don't want to have it at all - we should put it in a comment.
					document.getElementById("buttonID").style.visibility = 'hidden'; // hiding the submit button
...
<script type="text/javascript">		
	...
	function hideimage() {
		...
		document.getElementById('buttonID').style.display = 'block'; // relevant for the status pages - the submit button appears
...
<tr align="center" align="20px">
	...
	<?php if($signal==1){
			...
			onclick="var numValHoriz=leftanswer();">';?>
	...
	<td><?php if($signal==2){
			...
			style="background:#DCDCDC; font-size:45px; height:80px; width:80px; margin-left:auto; margin-right:auto; display:none"
			onclick="var numValVertic=topanswer();">';}
	?>
	</td> 
	<td rowspan="3"></td>
	<td rowspan="3">
	<?php if($signal==1){
			...
			style="background:#DCDCDC; font-size:45px; height:80px; width:80px; margin-left:auto; margin-right:auto; display:none"
			onclick="var numValHoriz=rightanswer();">';}
	?>
	</td>
</tr>
  
<tr align="center" align="20px" height="400">
	<td>

	
	<canvas id="myCanvas" width="400" height="400" style="border:0px solid #d3d3d3; display:none;"></canvas>
	<img src="smileyhappy.png" id="happy"  style="display:none;">
	<img  src="smileysad.png" id="sad"  style=" display:none;">
	<img  src="smileyempty.png" id="empty"  style=" display:none;">
...
<input type="hidden" name="correctIsEqual" value="<?php print $correctIsEqual?>">
...
<input type="hidden" name="arrayTrue1ImpHorRightString" value="<?php print $arrayTrue1ImpHorRightString?>">
<input type="hidden" name="arrayTrue1ImpHorLeftString" value="<?php print $arrayTrue1ImpHorLeftString?>">
<input type="hidden" name="arrayTrue1ImpVerTopString" value="<?php print $arrayTrue1ImpVerTopString?>">
<input type="hidden" name="arrayTrue1ImpVerBottomString" value="<?php print $arrayTrue1ImpVerBottomString?>">
...
<input type="hidden" id="input9" name="devicePixelRatio">
...
<tr align="center" align="20px">
	<td><?php if($signal==2){
			...
			style="background:#DCDCDC; font-size:45px; height:80px; width:80px; margin-left:auto; margin-right:auto; display:none"
			onclick="var numValVertic=bottomanswer();">';}
	?>
	</td>
</tr>
...
<script type="text/javascript">
	...
	var date21stPress;
	var b1stPress;
	var trialLevelJS = '<?php print $trialLevel?>';
	var whichIsBiggerJS = '<?php print $whichIsBigger?>';
	
	var impManipHor='<?php print $impManipHor?>';
	var impManipVert='<?php print $impManipVert?>';
...
function pseudoSubmit(){
	...
	setTimeout(function(){ document.forms["myForm"].submit(); }, document.getElementById("delayTimeMS").value);
		var signalJS = '<?php print $signal?>';
		var checkIfSignal999JS='<?php print $checkIfSignal999?>';
		var canvas = document.getElementById("myCanvas");
		var arrayTrue1ImpHorRight = [7,13,25,37,49,61,85,91,115,127,133,145,157,175,193,199,205,217,223,247,253,265,277,289,301,325,331,355,367,373,385,397,415,433,439,445,457,463];
		var arrayTrue1ImpHorLeft = [67,307];
		var arrayTrue1ImpVerTop = [16,28,40,46,58,64,88,100,118,124,136,142,154,172,190,208,214,226,232,256,268,280,286,298,304,328,340,358,364,376,382,394,412,430,448,454,466,472];
		var arrayTrue1ImpVerBottom = [76,316];
		if( realTrialJS>= 1 && checkIfSignal999JS==0 ){
			if(
			(trialLevelJS=="possibleHorizontal" && whichIsBiggerJS=="left" && numberValue==-1) || 
			(trialLevelJS=="possibleHorizontal" && whichIsBiggerJS=="right" && numberValue==1) || 
			(trialLevelJS=="possibleVertical" && whichIsBiggerJS=="top" && numberValue==1) || 
			(trialLevelJS=="possibleVertical" && whichIsBiggerJS=="bottom" && numberValue==-1) || 
			(trialLevelJS=="impossibleHorizontal" && arrayTrue1ImpHorRight.includes(realTrialJSnum) && ((numberValue*impManipHor)==1)) || 
			(trialLevelJS=="impossibleHorizontal" && arrayTrue1ImpHorLeft.includes(realTrialJSnum) && ((numberValue*impManipHor)==-1)) || 
			(trialLevelJS=="impossibleVertical" && arrayTrue1ImpVerTop.includes(realTrialJSnum) && ((numberValue*impManipVert)==1)) || 
			(trialLevelJS=="impossibleVertical" && arrayTrue1ImpVerBottom.includes(realTrialJSnum) && ((numberValue*impManipVert)==-1)) 
			){
			...
			feedback(canvas,correct_or_incorrect);
			function feedback(canvas,correct_or_incorrect) {
				canvas.style.display='block';
				var ctx = canvas.getContext("2d");
				ctx.fillStyle = "white";
				if(correct_or_incorrect == "correct"){
					ctx.fillText("Correct!", 150,90);
					var imgHappy = document.getElementById("happy");
					ctx.drawImage(imgHappy, 10, 10);
				}
				if(correct_or_incorrect == "incorrect"){
					ctx.fillText("Incorrect!", 150,90); 
					var imgSad = document.getElementById("sad");
					ctx.drawImage(imgSad, 10, 10);
				}
				if(correct_or_incorrect == "emptyfeedback"){
					ctx.fillText("Wait!", 150,90); 
					var imgEmpty = document.getElementById("empty");
					ctx.drawImage(imgEmpty, 10, 10);
				}
			}
...
function verticalFixation($realTrial,$arrayDevVertical,$arrayLocVertical){
	...
	$y1_horizontal=$devFix+110; //because the line normally appears above
	$y2_horizontal=$devFix+110; //because the line normally appears above
	
	$y1_vertical=$devFix-0.5*$fixLength+110; //because the line normally appears above
	$y2_vertical=$devFix+0.5*$fixLength+110; //because the line normally appears above
	$x1_vertical=$locFix;
	$x2_vertical=$locFix;
