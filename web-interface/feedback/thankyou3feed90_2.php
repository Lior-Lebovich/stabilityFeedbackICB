...
<h1 style="color:#000000; font-size:20px; margin-left:auto; margin-right:auto">
	<br><br>
	Thank you for participating in the second experiment!<br><br>
...
<h3 style="color:#000000; font-size:24px; margin-left:auto; margin-right:auto">
	Please use the following link to start the third experiment:<br>
	<a href="https://.com/f3/pre_form_3feed90_3.php" style="color:#1E90FF; font-size:22px; margin-left:auto; margin-right:auto">
	https://.com/f3/pre_form_3feed90_3.php
	</a>
</h3>

<h4 style="color:#000000; font-size:20px; margin-left:auto; margin-right:auto">
	Note that using that link, you can also check when should you start the third experiment by entering your worker ID.<br><br>
...
// If email does not exist in table -->  if not provided in sess2 - take the email in sess1; else - take the email of sess2.
if( empty($emailSQL) && ($sumResults_exist_email==0) ){
	// search for email in previous table:
	$connPrevTable = mysqli_connect($servername, $username, $password, "timescale3feed901");
	$sqlPrevTable = "SELECT DISTINCT email FROM expcomments WHERE workerID = '".substr($workerID, 0, 50)."' AND workerID NOT IN('')";
	$rowPrevTable = mysqli_fetch_array(mysqli_query($connPrevTable, $sqlPrevTable));
	$emailPrevious=$rowPrevTable['email'];
	$emailToSend = $emailPrevious;
	mysqli_close($connPrevTable);
} 
elseif( !empty($emailSQL) && ($sumResults_exist_email==0) ){
	$emailToSend = $emailSQL;
}

// If the email taken is not empty - save to mailF4:
if( !empty($emailToSend) ){
	$whetherToSend = 0;
	$sql_email = "INSERT INTO MailF4(timeF,emailF,isSentEmail,ID,experimentType) VALUES(NOW(),'$emailToSend','$whetherToSend','$ID26','$experimentType')";
