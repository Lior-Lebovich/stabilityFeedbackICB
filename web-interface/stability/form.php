...
$sumResults = mysqli_num_rows($results)+mysqli_num_rows($results2);
mysqli_close($conn1);
mysqli_close($conn2);
if( $sumResults>=1 )
{
	$continueExp = "workerid.php";
	$good=0;
}
else{
	$continueExp = "instructions.php";
	$good=1;
}
?>
...
<form id="myForm" name="myForm" class="form-horizontal" action="instructions.php" method="post">

<p id="intro2" name="intro2" class="form_div">
	Your worker ID exists in our database, probably because you have already participated in this experiment or in a similar version of it.<br><br>
	Multiple submissions with the same worker ID are not allowed.<br><br>
	See you on our next experiments.
</p>
