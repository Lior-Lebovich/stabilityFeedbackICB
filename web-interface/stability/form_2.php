...
$conn1 = mysqli_connect($servername, $username, $password, "timescalegood");
if (!$conn) {
...
(TIMESTAMPDIFF(MINUTE, endDate, NOW())/60) AS hourDiff 
FROM data2 
WHERE workerID = '".$workerID."' 
ORDER BY hourDiff ASC LIMIT 1";
...
if($experimentTypePrev=="1month"){
	$dayCond = 30;
}
elseif($experimentTypePrev=="3month"){
	$dayCond = 90;
}
elseif($experimentTypePrev=="6month"){
	$dayCond = 180+60;
}
else{
	$dayCond = 30;
...
$idealHourDiff = 24*($dayCond-10);
$maxHourDiff = 24*($dayCond+10);
$HourDiff2Start = $idealHourDiff-$deltaHour;
if( $sumResults>=1 && ( ($deltaHour >= $idealHourDiff) && ($deltaHour <= $maxHourDiff)  ) )
{
...
<form id="myForm" name="myForm" class="form-horizontal" action="instructions_2.php" method="post">
...
<p id="intro2" name="intro2" class="form_div">
	...
	<a href="http://.com/pre_form_time_2.php">
